...
# FastAPI app
# =========================
app = FastAPI(title="Image Uploader to S3 (with Nudity Check)", version="1.1.0")
...
def _requests_session_with_retries() -> requests.Session:
    ...
    retry = Retry(
        ...
        allowed_methods=frozenset(["POST"]),  # only POST here
...
async def upload_image(
    ...
    # 2.1) Enforce JPEG/PNG only using magic bytes (do not trust client headers)
    ...
    # Optional: also verify filename extension roughly matches the detected type
    # (You can keep it lenient; main guard is magic bytes above.)
    ext_from_map = ALLOWED_CONTENT_TYPES[detected_ct]
